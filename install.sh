...
readonly SP_BASE_DOWNLOAD_URL="${SERVERPILOT_DOWNLOAD_URL:-${AIINSTALLER_DOWNLOAD_URL:-https://github.com/serverpilot/serverpilot/releases/latest/download}}"
readonly SP_DEFAULT_SERVER_URL="${SERVERPILOT_SERVER:-${AIINSTALLER_SERVER:-wss://api.serverpilot.dev}}"
...
main() {
  ...
  # 1. Detect platform
  local os arch filename distro pkg_mgr
  os="$(detect_os)"
  arch="$(detect_arch)"
