...
describe('packages/server/package.json', () => {
  const pkg = readJSON(PACKAGE_JSON_PATH) as Record<string, unknown>;

  describe('Basic fields', () => {
    it('should have correct name', () => {
      expect(pkg.name).toBe('@aiinstaller/server');
    });
  ...
  describe('Dependencies', () => {
    ...
    it('should depend on @anthropic-ai/sdk', () => {
      expect(deps()['@anthropic-ai/sdk']).toBeDefined();
      expect(deps()['@anthropic-ai/sdk']).toMatch(/^\^/);
    });

    it('should depend on ws', () => {
      expect(deps().ws).toBeDefined();
      expect(deps().ws).toMatch(/^\^8/);
    });

    it('should depend on hono', () => {
      expect(deps().hono).toBeDefined();
      expect(deps().hono).toMatch(/^\^4/);
    });

    it('should depend on zod', () => {
      expect(deps().zod).toBeDefined();
      expect(deps().zod).toMatch(/^\^3/);
    });

    it('should depend on dotenv', () => {
      expect(deps().dotenv).toBeDefined();
      expect(deps().dotenv).toMatch(/^\^16/);
    });
  });
...
describe('packages/server/tsconfig.json', () => {
...
describe('packages/server directory structure', () => {
  it('should have src/ directory', () => {
    expect(existsSync(join(SERVER_DIR, 'src'))).toBe(true);
    expect(statSync(join(SERVER_DIR, 'src')).isDirectory()).toBe(true);
  });
  ...
  it('should have src/api/ directory', () => {
    expect(existsSync(join(SERVER_DIR, 'src', 'api'))).toBe(true);
  });

  it('should have src/ai/ directory', () => {
    expect(existsSync(join(SERVER_DIR, 'src', 'ai'))).toBe(true);
  });

  it('should have src/knowledge/ directory', () => {
    expect(existsSync(join(SERVER_DIR, 'src', 'knowledge'))).toBe(true);
  });
});
...
describe('Workspace integration', () => {
  ...
  it('should be referenced in root tsconfig paths', () => {
    ...
    expect(paths['@aiinstaller/server']).toBeDefined();
    expect(paths['@aiinstaller/server']).toContain('packages/server/src');
  });

  it('should have root package.json scripts referencing server', () => {
    ...
    expect(scripts['dev:server']).toContain('@aiinstaller/server');
    expect(scripts['build:server']).toContain('@aiinstaller/server');
  });
});

describe('Consistency with shared package', () => {
  const serverPkg = readJSON(PACKAGE_JSON_PATH) as Record<string, unknown>;
  ...
  it('should use same TypeScript version range as shared package', () => {
    ...
    expect(serverDevDeps.typescript).toBe(sharedDevDeps.typescript);
  });

  it('should use same Vitest version range as shared package', () => {
    const serverDevDeps = serverPkg.devDependencies as Record<string, string>;
    const sharedDevDeps = sharedPkg.devDependencies as Record<string, string>;
    expect(serverDevDeps.vitest).toBe(sharedDevDeps.vitest);
  });

  it('should use same Node.js engine requirement', () => {
    const serverEngines = serverPkg.engines as Record<string, string>;
    const sharedEngines = sharedPkg.engines as Record<string, string>;
    expect(serverEngines.node).toBe(sharedEngines.node);
  });

  it('should use same ESM module type', () => {
    expect(serverPkg.type).toBe(sharedPkg.type);
  });
});
