...
describe("docker-compose.yml ↔ .env.example consistency", () => {
  it("all compose env vars with defaults should have matching .env.example entries", () => {
    ...
    for (const match of composeVars) {
      ...
      expect(
        envExample,
        `${varName} should be documented in .env.example`,
      ).toContain(varName);
    }
  });
  ...
  it("WS_HEARTBEAT_INTERVAL_MS default should be consistent", () => {
    const composeVal = compose.match(
      /WS_HEARTBEAT_INTERVAL_MS=\$\{WS_HEARTBEAT_INTERVAL_MS:-(\d+)\}/,
    );
  ...
  it("WS_CONNECTION_TIMEOUT_MS default should be consistent", () => {
    const composeVal = compose.match(
      /WS_CONNECTION_TIMEOUT_MS=\$\{WS_CONNECTION_TIMEOUT_MS:-(\d+)\}/,
    );
...
describe("Dockerfile ↔ project configuration consistency", () => {
  it("Node.js version in Dockerfile should match engines requirement", () => {
    ...
    expect(majorVersion).toBeGreaterThanOrEqual(
      parseInt(requiredVersion![1], 10),
    );
  });

  it("pnpm version in Dockerfile should match packageManager field", () => {
    // Dockerfile uses: npm install -g pnpm@VERSION or corepack prepare pnpm@VERSION
    const dockerPnpm = dockerfile.match(
      /(?:npm install -g |corepack prepare )pnpm@([\d.]+)/,
    );
...
describe(".dockerignore completeness", () => {
  it("should not exclude files required by the build", () => {
    ...
    for (const file of requiredFiles) {
      ...
      expect(
        directlyIgnored,
        `${file} should NOT be in .dockerignore (needed for build)`,
      ).toBe(false);
    }
  });
...
describe("image size optimization", () => {
  ...
  it("should install only production dependencies in runtime stage", () => {
    // Runtime stage should install only production deps (either via prune or --prod flag)
    const runtimeStageMatch = dockerfile.match(/AS\s+runtime[\s\S]*/);
    expect(runtimeStageMatch).not.toBeNull();
    // Should use either 'prune --prod' or 'install --prod'
    const hasProductionDeps =
      runtimeStageMatch![0].includes("prune --prod") ||
      runtimeStageMatch![0].includes("install --prod") ||
      runtimeStageMatch![0].includes("install --frozen-lockfile --prod");
    expect(hasProductionDeps).toBe(true);
  });
  ...
  it("should only copy dist output to runtime stage (not source)", () => {
    ...
    const srcCopies =
      runtimeContent.match(/COPY.*packages\/server\/src/g) || [];
    const nonMigrationSrcCopies = srcCopies.filter(
      (c) => !c.includes("migrations"),
    );
