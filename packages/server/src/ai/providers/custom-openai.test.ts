...
function createProvider(
  overrides?: Partial<CustomOpenAIConfig>,
): CustomOpenAIProvider {
...
describe("CustomOpenAIProvider", () => {
  ...
  describe("CustomOpenAIConfigSchema", () => {
    ...
    it("should reject missing baseUrl", () => {
      expect(() =>
        CustomOpenAIConfigSchema.parse({
          apiKey: "sk-test",
          model: "gpt-4o",
        }),
      ).toThrow();
  ...
  describe("isAvailable()", () => {
    ...
    it("should throw when API is unreachable", async () => {
      ...
      await expect(provider.isAvailable()).rejects.toThrow(/无法连接自定义/);
    });
  ...
  describe("CustomOpenAIError", () => {
    ...
    it("should have correct name for authentication error", () => {
      const err = new CustomOpenAIError(
        "Custom OpenAI authentication failed: Invalid API key",
        401,
      );
      expect(err.name).toBe("CustomOpenAIError");
    ...
    it("should have correct name for rate limit error", () => {
      const err = new CustomOpenAIError(
        "Custom OpenAI rate limit exceeded",
        429,
      );
