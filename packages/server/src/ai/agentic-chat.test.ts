            ...
            return {
              content: [
                { type: "text", text: `Thinking turn ${callIndex}...` },
                {
                  ...
                  id: `tool-${callIndex}`,
                  name: "execute_command",
                  input: { command: "echo test", description: "test" },
                },
              ],
...
describe("AgenticChatEngine — malformed tool input", () => {
  ...
  function _createClientWithMalformedToolInput(
    toolName: string,
          ...
          return {
            ...
            finalMessage: vi.fn(async () => ({
              content: [
                {
                  type: "text",
                  text: "I see the input was invalid, let me fix that.",
                },
              ],
              stop_reason: "end_turn",
            })),
  ...
  it("should return error string and send validation_error SSE for execute_command with numeric command", async () => {
    ...
    const validationEvents = sseEvents.filter(
      (e) =>
        e.event === "tool_result" &&
        (e.data as Record<string, unknown>).status === "validation_error",
    );
    expect(validationEvents).toHaveLength(1);
    expect((validationEvents[0].data as Record<string, unknown>).tool).toBe(
      "execute_command",
    );
    expect((validationEvents[0].data as Record<string, unknown>).id).toBe(
      "tool-bad",
    );
    expect(
      (validationEvents[0].data as Record<string, unknown>).error,
    ).toBeDefined();
  });

  it("should send validation_error SSE for execute_command with missing fields", async () => {
    const { client, getCallCount } = createClientTwoTurns(
      "execute_command",
      {},
    );
    const engine = new AgenticChatEngine(client);
  ...
  it("should send validation_error SSE for read_file with missing path", async () => {
    const { client, getCallCount } = createClientTwoTurns("read_file", {
      max_lines: 100,
    });
    const engine = new AgenticChatEngine(client);
    const { stream, sseEvents } = createMockStream();
  ...
  it("should send validation_error SSE for list_files with non-string path", async () => {
    const { client, getCallCount } = createClientTwoTurns("list_files", {
      path: 42,
    });
    const engine = new AgenticChatEngine(client);
    const { stream, sseEvents } = createMockStream();
  ...
  it("should log a warning when tool input validation fails", async () => {
    ...
    const { client } = createClientTwoTurns("execute_command", {
      command: 999,
    });
    const engine = new AgenticChatEngine(client);
    const { stream } = createMockStream();
                ...
                content: [
                  {
                    ...
                    id: "tool-1",
                    name: "execute_command",
                    input: { command: "echo 1", description: "first" },
                  },
                  {
                    type: "tool_use",
                    id: "tool-2",
                    name: "execute_command",
                    input: { command: "echo 2", description: "second" },
                  },
                  {
                    type: "tool_use",
                    id: "tool-3",
                    name: "execute_command",
                    input: { command: "echo 3", description: "third" },
                  },
                ],
                stop_reason: "tool_use",
              })),
...
describe("AgenticChatEngine — confirmation abort race", () => {
  ...
  it("should unblock confirmation wait within 1s when client disconnects", async () => {
    ...
    const onConfirmRequired = vi.fn(
      (_cmd: string, _risk: string, _desc: string) => ({
    ...
    const confirmEvents = sseEvents.filter(
      (e) => e.event === "confirm_required",
    );
...
describe("AgenticChatEngine — pre-trim long conversation history", () => {
  ...
  it("should not fail on first API call when conversation history exceeds 150K tokens", async () => {
    ...
    for (let i = 0; i < 400; i++) {
      longHistory.push({
        role: "user",
        content: `用户消息 ${i}: ${"这是一段很长的对话内容".repeat(50)}`,
      });
      longHistory.push({
        role: "assistant",
        content: `助手回复 ${i}: ${"执行系统检查和维护操作".repeat(50)}`,
      });
    }
  ...
  it("should preserve the first user message (original context) after pre-trim", async () => {
    ...
    for (let i = 0; i < 400; i++) {
      longHistory.push({
        role: "user",
        content: `Msg ${i}: ${"z".repeat(2000)}`,
      });
      longHistory.push({
        role: "assistant",
        content: `Rep ${i}: ${"w".repeat(2000)}`,
      });
    }
  ...
  it("should handle CJK conversation history pre-trim correctly", async () => {
    ...
    for (let i = 0; i < 300; i++) {
      cjkHistory.push({
        role: "user",
        content: `用户请求 ${i}: ${"服务器运维管理系统检查".repeat(100)}`,
      });
      cjkHistory.push({
        role: "assistant",
        content: `系统回复 ${i}: ${"正在执行安全检查和性能优化".repeat(100)}`,
      });
    }
              ...
              content: [
                {
                  ...
                  name: "execute_command",
                  input: {
                    command: `echo step${callIndex}`,
                    description: `Step ${callIndex}`,
                  },
                },
              ],
              stop_reason: "tool_use",
...
// ============================================================================
// getUserFacingErrorMessage — error type → actionable user message (chat-098)
// ============================================================================

describe("getUserFacingErrorMessage", () => {
  it("should return API key advice for 401 authentication errors", () => {
    const error = Object.assign(new Error("Unauthorized"), { status: 401 });
    const msg = getUserFacingErrorMessage(error);
    expect(msg).toContain("API Key");
    expect(msg).toContain("无效或已过期");
  });

  it("should return API key advice for 403 permission errors", () => {
    const error = Object.assign(new Error("Forbidden"), { status: 403 });
    const msg = getUserFacingErrorMessage(error);
    expect(msg).toContain("API Key");
  });

  it("should return rate limit advice for 429 errors", () => {
    const error = Object.assign(new Error("Too Many Requests"), {
      status: 429,
    });
    const msg = getUserFacingErrorMessage(error);
    expect(msg).toContain("请求过于频繁");
  ...
  it("should return timeout advice for timeout errors", () => {
  ...
  it("should return overloaded advice for 503 errors", () => {
  ...
  it("should return server error advice for 500 errors", () => {
  ...
  it("should include raw message for unknown errors", () => {
    ...
    const msg = getUserFacingErrorMessage(error);
    expect(msg).toContain("执行过程中发生错误");
    expect(msg).toContain("Something completely unexpected");
  });

  it("should handle non-Error objects gracefully", () => {
    const msg = getUserFacingErrorMessage("plain string error");
    expect(msg).toContain("执行过程中发生错误");
...
// ============================================================================
// AgenticChatEngine — error classification in agentic loop (chat-098)
// ============================================================================

describe("AgenticChatEngine — classified error messages", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("should send authentication-specific message for 401 API errors", async () => {
    ...
    const errorMessage = messageEvents.find(
      (e) =>
        typeof e.data.content === "string" &&
        (e.data.content as string).includes("API Key"),
    );
    expect(errorMessage).toBeDefined();

    // Complete event should still contain raw error for debugging
    const completeEvent = sseEvents.find((e) => e.event === "complete");
    expect(completeEvent).toBeDefined();
    expect(completeEvent!.data.error).toBe("Invalid API key");
  });

  it("should send rate limit message for 429 errors", async () => {
    ...
    const errorMessage = messageEvents.find(
      (e) =>
        typeof e.data.content === "string" &&
        (e.data.content as string).includes("请求过于频繁"),
    );
    expect(errorMessage).toBeDefined();
  });

  it("should send new session advice for context length errors (400)", async () => {
    const error = Object.assign(new Error("context_length_exceeded"), {
      status: 400,
    });
    const client = createFailingClient(error);
    ...
    const errorMessage = messageEvents.find(
      (e) =>
        typeof e.data.content === "string" &&
        (e.data.content as string).includes("新建会话"),
    );
    expect(errorMessage).toBeDefined();
  });

  it("should send generic message with details for unknown errors", async () => {
    ...
    const errorMessage = messageEvents.find(
      (e) =>
        typeof e.data.content === "string" &&
        (e.data.content as string).includes("Something went wrong"),
    );
    expect(errorMessage).toBeDefined();
  });

  it("should still log detailed error to logger regardless of category", async () => {
    const { logger: loggerModule } = await import("../utils/logger.js");
    const errorSpy = vi.spyOn(loggerModule, "error");

    const error = Object.assign(new Error("token limit exceeded"), {
      status: 429,
    });
    const client = createFailingClient(error);
    const engine = new AgenticChatEngine(client);
