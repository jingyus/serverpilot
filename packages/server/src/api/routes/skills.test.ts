...
describe('GET /skills/:id/export', () => {
  it('should return a .tar.gz archive with correct headers', async () => {
    ...
    expect(res.headers.get('Content-Disposition')).toBe('attachment; filename="nginx-hardening-1.0.0.tar.gz"');
    expect(res.headers.get('Content-Length')).toBe(String(archiveBuffer.length));
  ...
  it('should return 404 for nonexistent skill', async () => {
    mockExportSkill.mockRejectedValue(new Error('Skill not found: nonexistent'));
...
describe('POST /skills/import', () => {
  ...
  it('should import a skill from .tar.gz and return 201', async () => {
    ...
    const formData = createFormData('nginx-hardening-1.0.0.tar.gz', Buffer.from('fake-archive'));
  ...
  it('should return warnings from security scan', async () => {
    ...
    const formData = createFormData('test-skill-1.0.0.tar.gz', Buffer.from('archive-data'));
  ...
  it('should accept .tgz file extension', async () => {
    ...
    const formData = createFormData('nginx-hardening-1.0.0.tgz', Buffer.from('tgz-archive'));
  ...
  it('should return 400 for duplicate skill import', async () => {
    ...
    const formData = createFormData('nginx-hardening-1.0.0.tar.gz', Buffer.from('archive'));
  ...
  it('should return 400 when archive has no valid skill', async () => {
    ...
    const formData = createFormData('bad-archive-1.0.0.tar.gz', Buffer.from('bad'));
  ...
  it('should return 400 when security scan fails', async () => {
    mockImportSkill.mockRejectedValue(
      new Error('Security scan failed for imported skill: contains malicious pattern'),
    );

    const formData = createFormData('malicious-1.0.0.tar.gz', Buffer.from('malicious'));
  ...
  it('should return 400 when target directory already exists', async () => {
    mockImportSkill.mockRejectedValue(
      new Error('Target directory already exists: /skills/community/nginx-hardening'),
    );
  ...
  it('should propagate unexpected errors as 500', async () => {
    ...
    const formData = createFormData('test-1.0.0.tar.gz', Buffer.from('archive'));
