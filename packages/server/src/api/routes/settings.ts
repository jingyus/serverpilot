...
settings.put(
  ...
  validateBody(UpdateAIProviderBodySchema),
  async (c) => {
    const userId = c.get("userId");
    ...
    try {
      ...
      throw ApiError.badRequest(
        `Failed to initialize provider "${body.provider}": ${msg}`,
      );
    }
...
settings.put(
  "/notifications",
  requirePermission("settings:update"),
  validateBody(UpdateNotificationsBodySchema),
  async (c) => {
    const userId = c.get("userId");
...
settings.put(
  "/knowledge-base",
  requirePermission("settings:update"),
  validateBody(UpdateKnowledgeBaseBodySchema),
  async (c) => {
    const userId = c.get("userId");
