...
function createMockProvider(
  overrides?: Partial<AIProviderInterface>,
): AIProviderInterface {
  return {
    name: "mock-provider",
    tier: 1 as const,
    ...
    stream: vi
      .fn<[ChatOptions, ProviderStreamCallbacks?], Promise<StreamResponse>>()
      .mockResolvedValue({
...
describe("ChatAIAgent", () => {
  ...
  describe("chat() — success", () => {
    ...
    it("passes conversation history into user prompt", async () => {
      ...
      const streamCall = (provider.stream as ReturnType<typeof vi.fn>).mock
        .calls[0];
  ...
  describe("chat() — retry logic", () => {
    ...
    it("throws immediately on non-retryable error (auth)", async () => {
      ...
      (provider.stream as ReturnType<typeof vi.fn>).mockRejectedValueOnce(
        authError,
      );

      await expect(agent.chat("msg", "Server: x", "")).rejects.toThrow(
        "Invalid API key",
      );
    ...
    it("throws ChatRetryExhaustedError after all retries", async () => {
      ...
      (provider.stream as ReturnType<typeof vi.fn>).mockRejectedValue(
        serverError,
      );
    ...
    it("handles stream returning success=false as an error", async () => {
      mockClassifyError({
        retryable: false,
        category: "unknown",
        message: "AI streaming request failed",
      });
  ...
  describe("chatWithFallback()", () => {
    ...
    it("returns null when fallback provider also fails", async () => {
      ...
      (failingProvider.stream as ReturnType<typeof vi.fn>).mockRejectedValue(
        new Error("Bad key"),
      );
      (createProvider as ReturnType<typeof vi.fn>).mockReturnValueOnce(
        failingProvider,
  ...
  describe("ChatRetryExhaustedError", () => {
    it("sets name and classification properties", () => {
      ...
      const err = new ChatRetryExhaustedError(
        "All retries failed",
        classification,
      );
  ...
  it("falls back through priority order: claude → openai → deepseek → ollama", () => {
    ...
    (createProvider as ReturnType<typeof vi.fn>)
      .mockImplementationOnce(() => {
        throw new Error("No openai credentials");
      }) // openai skipped (no key, but it still goes through the flow)
