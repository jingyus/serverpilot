...
import { getActiveProvider, checkProviderHealth } from '../../ai/providers/provider-factory.js';
...
function checkDatabase(): DatabaseHealth {
  try {
    ...
    return { status: 'unhealthy', type: 'sqlite', message: 'Database not initialized or unreachable' };
  }
}
...
function checkRag(): RagHealth {
  ...
  return {
    ...
    message: pipeline.isReady() ? undefined : 'RAG pipeline not ready (no indexed documents)',
  };
}

function aggregateStatus(subsystems: HealthDetailResponse['subsystems']): OverallStatus {
  const statuses = [
