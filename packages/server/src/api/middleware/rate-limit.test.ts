...
function createTestApp(config?: Parameters<typeof createRateLimitMiddleware>[0], routeOverrides?: Parameters<typeof createRateLimitMiddleware>[1]) {
...
describe('rateLimit middleware', () => {
  ...
  describe('authenticated requests (user-based)', () => {
    function createAuthApp(config?: Parameters<typeof createRateLimitMiddleware>[0]) {
      const app = new Hono<ApiEnv>();
      app.onError(onError);
  ...
  describe('route-specific overrides', () => {
    it('should apply stricter limit to login route', async () => {
      const { app } = createTestApp({ anonymousLimit: 100 });

      // Login has default override of 5/min
      for (let i = 0; i < 5; i++) {
    ...
    it('should apply stricter limit to register route', async () => {
      const { app } = createTestApp({ anonymousLimit: 100 });
  ...
  describe('window reset', () => {
    it('should allow requests after window expires', async () => {
      ...
      try {
        const app = new Hono<ApiEnv>();
        app.onError(onError);
        app.use('/api/v1/*', createRateLimitMiddleware({ anonymousLimit: 2, windowMs: 1000, cleanupIntervalMs: 500 }, []));
