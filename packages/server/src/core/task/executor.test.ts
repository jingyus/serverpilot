...
// Mock command approval service to avoid database dependency
vi.mock("../approvals/approval-service.js", () => ({
  getCommandApprovalService: vi.fn(() => ({
...
function makeCommandInput(
  overrides: Partial<ExecuteCommandInput> = {},
): ExecuteCommandInput {
  return {
    serverId: "srv-1",
    userId: "user-1",
...
describe("executeCommand", () => {
  ...
  it("should resolve with success result when agent reports success", async () => {
    ...
    executor.handleStepComplete(
      makeStepResult(stepId, {
        stdout: "hello\n",
        stderr: "",
        exitCode: 0,
        success: true,
...
describe("executePlan", () => {
  it("should execute steps sequentially", async () => {
    ...
    executor.handleStepComplete(
      makeStepResult(send1[1].payload.id, {
        stdout: "step1\n",
      }),
    );
    ...
    executor.handleStepComplete(
      makeStepResult(send2[1].payload.id, {
        stdout: "step2\n",
      }),
    );
...
describe("snapshot integration", () => {
  function createMockSnapshotService(
    ...
    return {
      ...
      createPreOperationSnapshot: vi.fn(
...
describe("progress listeners", () => {
  ...
  it("should coexist with legacy setProgressCallback", async () => {
    ...
    expect(legacyFn).toHaveBeenCalledWith(
      expect.any(String),
      "running",
      undefined,
    );
  ...
  it("should clear all listeners on shutdown", async () => {
    ...
    const { promise: p2, stepId: s2 } = await startCommandAndWaitForSend(
      executor2,
      server,
    );
  ...
  it("should support concurrent executions with per-listener progress", async () => {
    ...
    const p1 = executor.executeCommand(
      makeCommandInput({ clientId: "c1", command: "cmd1" }),
    );
    const p2 = executor.executeCommand(
      makeCommandInput({ clientId: "c2", command: "cmd2" }),
    );
    await flushMicrotasks();
