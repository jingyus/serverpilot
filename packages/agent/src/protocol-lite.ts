 ...
 */
export function safeParseMessageLite(
  data: unknown,
): { success: true; data: Message } | { success: false; error: Error } {
  if (data === null || data === undefined || typeof data !== "object") {
    return {
      success: false,
      error: new Error("Message must be a non-null object"),
    };
  }
  ...
  if (typeof msg.type !== "string") {
    return {
      success: false,
      error: new Error('Message must have a string "type" field'),
    };
  }
  ...
  if (!validTypes.includes(msg.type)) {
    return {
      success: false,
      error: new Error(`Unknown message type: ${msg.type}`),
    };
  }
  ...
  if (typeof msg.timestamp !== "number") {
    return {
      success: false,
      error: new Error('Message must have a numeric "timestamp" field'),
    };
  }
