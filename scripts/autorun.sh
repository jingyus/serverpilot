...
- **⚠️ 单包原则（必须遵守）**: 每个任务只能修改**一个**包（server 或 dashboard 或 shared 或 agent），禁止生成跨包任务
- **影响范围限制**: 每个任务最多修改 3-4 个文件
- **跨包功能必须拆分**: 如需跨包实现（如新 schema + server 逻辑 + dashboard UI），必须拆成多个独立任务，按依赖顺序排列：先 shared → 再 server → 最后 dashboard。每个子任务独立可测试
